...
export function OutletListPage(props: OutletListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getOutlets({
        relations: ['supplier', 'user', 'logistics.count', 'tool.count'],
        limit: params.pageSize,
        offset: params.pageNumber * params.pageSize,
        searchTerm: params.searchTerm,
        order: params.order,
        searchTermKeys: ['location.contains', 'phone_number.contains', 'email.contains', 'operating_hours.contains'],
        ...(params.filters || {}),
      }),
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'operating_hours', header: 'Operating Hours', accessorKey: 'operating_hours' },
    hasAccess('supplier', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/suppliers/view/${record.supplier?.id}`}>
