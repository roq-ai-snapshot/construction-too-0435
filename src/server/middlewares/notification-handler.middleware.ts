...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'Order #12345 has been placed by John Doe..create': {
    roles: ['business-owner', 'store-manager', 'customer-service-representative'],
    key: 'order_create',
    tenantPath: [],
    userPath: [],
  },
  'Store Manager Jane Doe has updated the inventory for Product XYZ..update': {
    roles: ['business-owner', 'store-manager'],
    key: 'inventory_update',
    tenantPath: [],
    userPath: [],
  },
  'Customer Service Representative John Smith has resolved complaint #12345..update': {
    roles: ['business-owner', 'store-manager', 'customer-service-representative'],
    key: 'complaint_resolve',
    tenantPath: [],
    userPath: [],
  },
};
...
export async function notificationHandlerMiddleware(req: NextApiRequest, entityId: string) {
  ...
  const tenant = await prisma.supplier.findFirst({
