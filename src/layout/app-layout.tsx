...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const { data, error, isLoading } = useSWR<SupplierInterface[]>(
    ...
    () => getSuppliers({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    { name: 'Logistics', path: '/logistics', entity: 'logistics', service: AccessServiceEnum.PROJECT, icon: FiTruck },
    {
